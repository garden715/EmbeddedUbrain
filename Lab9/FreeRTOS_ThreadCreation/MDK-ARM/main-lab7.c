...
void myTask(void *params) {
  ...
  int i;
    // Delay?? ???? tick???? 1000ms?? ???????.
	 const portTickType xDelay = 1000; //xDelay 1??? ???? 
...
void myTask2 (void) 	{ 
	...
	while (1) {
		/* Capture computation */
				...
				distance1 = uwDiffCapture1/58 ; // distance????
				distance2 = uwDiffCapture2/58 ; // distance????
				distance3 = uwDiffCapture3/58 ; // distance????
...
void myTask3(void) {
   ...
   adcConfig1.Rank = 1;
   adcConfig1.SamplingTime = ADC_SAMPLETIME_480CYCLES; //???©ª? ??? ????
   adcConfig1.Offset = 0;

   HAL_ADC_ConfigChannel(&AdcHandle1, &adcConfig1);
      
   AdcHandle2.Instance          = ADC2;   // ADC?¥ê?
   ...
   AdcHandle3.Instance          = ADC1;   // ADC?¥ê?
...
/*##-3- Start PWM signals generation #######################################*/ 
	...
	*/
	...
	// create and start Task - calcurate the distance
